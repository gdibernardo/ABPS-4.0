...
\\
Una volta che il frame sarà stato effettivamente spedito in seguito il sottosistema mac80211 riceverà lo status di consegna al first-hop per pacchetto.
\\
...
\begin{itemize}
\item Per segnalare a TED che l'applicativo invocante la system call sendmsg richiede di poter ricevere l'id per il messaggio in invio. 
\\
Per far ciò viene introdotto un nuovo valore non utilizzato per il campo \textbf{cmsg\_type} della struct cmsghdr.
...
Una volta che il messaggio sarà stato inviato tramite sendmsg il controllo passerà al Socket Interface Layer e in seguito sarà ripreso dalla primitiva di livello trasporto del kernel udp\_sendmsg  ( per ulteriori dettagli vedere capitolo 2 ).
\\
Attraverso una funzione appositamente sviluppata, si andranno ad analizzare i dati di tipo ancillary contenuti nel messaggio passato come parametro della sendmsg che sarà ricevuto de udp\_sendmsg.
\\
La struttura struct msghdr mantiene il messaggio passato alla system call dall'applicativo a livello utente e gli eventuali dati di controllo del messaggio. 
\\
...
\\
Il meccanismo è stato esteso copiando all'interno del socket buffer  valore dell'identificativo mantenuto dal datagramma originario: in questo modo ciascun fragment di uno stesso datagram IP avrà lo stesso identificativo del pacchetto iniziale.
\\
...
\\
Vengono memorizzate, inolte, delle informazioni aggiuntive relative alla frammentazione del messaggio originario:
\begin{itemize}
